...
ggplot(data = counts_since2010, aes(x = reorder(pais, average), y = average, fill=pais)) +
=======
ggplot(data = counts_since2010, aes(x = reorder(pais, average), y = average, fill=country)) +
>>>>>>> Stashed changes
  geom_bar(stat = "identity") +
  coord_flip() +
  geom_text(data = counts_since2010, aes(x = pais, y = average, label = average), vjust = -0.5, color = "black") +
...
counts_year[counts_year$country=='MEXICO',] %>% 
  ...
  annotate("text", x=2010, y=Y_VAL, label="2017 - \nTrump", color = COLOR, size = SIZE)+
  labs(title = 'Documentos presidenciales con mención de "México" ',
       subtitle = "Dividido por total de documentos ese año",
       y = "", 
       x = "", 
       caption = "*discursos disponibles en https://www.presidency.ucsb.edu/")
```
...
rm(temp_corpus, temp_tokens, temp_tokens_ngram, temp_dfm_tokens_ngram, temp_dfm_tokens)
```
