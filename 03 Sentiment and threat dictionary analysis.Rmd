...
tokens_df %>% 
  ...
  mutate(word_tokens = if_else(word_tokens == "problem" | word_tokens == "problems", "problem", word_tokens)) %>% #debemos juntar problem y problems
  count(word_tokens) %>% 
  arrange(desc(n)) %>%
  head(20) %>% 
...
```
#### Hacer lo mismo pero por partido
```{r}
...
total_toks_dem <- 500434
total_toks_rep <- 504478 
