...
ggplot(aes(x = fct_reorder(last_name, ndoc_prop), y = ndoc_prop, fill = Partido))+
  ...
  labs(title = 'Documentos presidenciales con mención de "México" ',
       ...
       y = "%", 
       x = "", caption = "*discursos disponibles") +
...
ndoc_year %>% 
  ...
  geom_vline(aes(xintercept = 1847), linetype = "dotted", color = "blue")+   
  ...
  annotate("text", x=1872, y=5.6, label="1862 - \n                         Maximiliano en México", color = "blue", size = 3)
  
  
  +
  geom_vline(aes(xintercept = 1971), linetype = "dotted")+   
  annotate("text", x=1969, y=30, label="1860", color = "blue", size = 3)+
  geom_vline(aes(xintercept = 2017), linetype = "dotted")+   
  annotate("text", x=2015, y=30, label="2017", color = "blue", size = 3)+
  #geom_text(aes(x = 1983, y = 120, label = "1986"), size = 3, color = "black")+
  labs(title = 'Number of Documents with "War on Drugs" related terms', 
       subtitle = "Proclamations, Spoken Addresses/Remarks, and Inaugural Addresses", 
       y = "Number of Documents", 
       x = "", 
       caption = '*In 1971 Nixon declares "War on Drugs" \n*In 1986 Reagan declares drug trade a "National Security Threat"\n*In 2017 Trump takes office')
```

## Converting dataframe to corpus

```{r,warning=FALSE}
corp_drug_speeches <- corpus(drugs_df, text_field = "text")
save(corp_drug_speeches,file = "data/corp_drug_speeches")
#summary(corp_drug_speeches)
class(corp_drug_speeches)
#as.character(corp_drug_speeches)[300]
```
...
```{r}
post_nixon <- corp_drug_speeches %>% 
  corpus_subset(year >= 1969)

nixon_reagan <- corp_drug_speeches %>% 
  corpus_subset(last_name %in% c("Nixon", "Reagan"))
