...
retrieve_number_of_records <- function(html_object){
  
  # Extract the number of records found from the page and remove any non-numeric characters
  raw_records <- html_object %>% html_node("#block-system-main h3") %>% html_text() %>% gsub("[^0-9]", "", .)

  # Convert records to numeric format if possible
  if (!is.na(raw_records) & nchar(raw_records) > 0) {
    raw_records <- as.numeric(raw_records)
  
    # Remove the 1-25 if it appears before the number of records
    if (!is.na(raw_records) & is.numeric(raw_records) & substr(raw_records, 1, 3) == "125") {
      records <- substr(raw_records, 4, nchar(raw_records))
    }
  }
  return (records)
}
